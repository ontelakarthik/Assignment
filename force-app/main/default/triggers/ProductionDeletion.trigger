    ...
    for(Product2 p : Trigger.old)
{
    ...
    List<OpportunityLineItem> usedProducts = [SELECT Id, Product2Id, Opportunity.StageName FROM OpportunityLineItem WHERE Product2Id IN :productIds AND Opportunity.StageName NOT IN ('Closed Won', 'Closed Lost')];
