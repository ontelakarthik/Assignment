trigger Summarytrigger on Opportunity (after update)
{
    ...
    for (AggregateResult ar : [
        SELECT Product2Id, SUM(TotalPrice) totalAmount, SUM(Quantity) totalUnits
    ...
    for (Id productId : amountMap.keySet()) {
        summaryList.add(new Product_Sales_Summary__c(
            Product__c = productId, 
            Product_Key__c = productId, 
            Total_Sales_Amount__c = amountMap.get(productId),
            Total_Units__c = qtyMap.get(productId)
        ));
    }
